    ...
    @include('profile.partials.nav') 
        ...
        <div class="card w-75 mt-4 rounded-2 p-6">
            <div class = "container w-75 p-4 bg-gray-200 rounded-md shadow-sm border-2">
                <form method="POST" action="{{ route('comp') }}" id="myForm">
                    @csrf
                    <div class="controls">
                        <div class="row">
                            <h1 class="mb-2 fs-3 text-center lead  fw-light text-gray-800 fw-medium">Compétences</h1>
                            <div class="col-md-12">
                                <div class="mt-4">
                                    <x-input-label for="competence" :value="__('Nom du compétence')" />
                                    <input class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" type="text" name="competence[]" :value="old('competence')" />
                                    @error('competence[]')
                                    <h1>{{$message}}</h1>
